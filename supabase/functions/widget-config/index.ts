...
import { handleOptions, json, badRequest, notFound } from "../_shared/cors.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const controls = await controlsForChatbot(ref.trim());
    ...
    const requestId = crypto.randomUUID(); console.error("widget-config error", { requestId, err });
    return json({ error: "Widget configuration unavailable", code: "WIDGET_CONFIG_500", requestId }, 500, { "X-Request-Id": requestId });
  }
});
