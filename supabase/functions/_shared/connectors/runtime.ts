...
export async function executeConnectorAction(opts:{tenantId:string;actionId:string;input:Record<string,unknown>;confirmed:boolean;userMessage:string;conversationId?:string}):Promise<{ok:boolean;text:string;interaction?:WidgetInteraction}>{
  ...
  if(action.provider==="resend"&&action.method==="POST"&&path==="/emails"){const fromEmail=String(creds.from_email??"").trim();const fromName=String(creds.from_name??"").trim();if(!fromEmail){await log(false,undefined,"RESEND_FROM_EMAIL_MISSING");return{ok:false,text:"Email sending is connected, but the sender email has not been configured."}}delete rest.from;rest.from=fromName?`${fromName} <${fromEmail}>`:fromEmail}
