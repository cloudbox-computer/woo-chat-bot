...
export class SupabaseDb implements Db {
  ...
  private async mapTenant(row: Record<string, unknown>): Promise<Tenant> {
    ...
    return {
      ...
      resendApiKey: await decryptSecret(resendCreds.api_key),
      resendFromEmail: resendCreds.from_email || undefined,
      resendFromName: resendCreds.from_name || undefined,
  ...
  async getTenantByChatbot(chatbotId: string): Promise<Tenant | null> {
    ...
    return rows[0] ? await this.mapTenant(rows[0]) : null;
  }
  ...
  async appendMessage(m: Message): Promise<void> {
    await this.insert("messages", {
      ...
      role: m.role,
      source: m.role === "user" ? "customer" : "ai",
      content: m.content,
  ...
  async nextTicketReference(tenantId: string, prefix: string): Promise<string> {
    const res = await fetch(`${this.base.replace(/\/rest\/v1$/, "")}/rest/v1/rpc/next_ticket_reference`, {
      method: "POST", headers: this.headers,
      body: JSON.stringify({ p_tenant: tenantId, p_prefix: prefix, p_year: new Date().getFullYear() }),
    });
    if (!res.ok) throw new Error(`DB next_ticket_reference: ${res.status} ${await res.text()}`);
