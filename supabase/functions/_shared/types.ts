...
export interface Tenant {
  ...
  supportEmail?: string;
  /** Tenant-specific Resend credentials. Loaded server-side from the integrations table. */
  resendApiKey?: string;
  ...
  /** Cached schema summary: { tables: [{ name, columns: [{name, type}] }] }.
   *  Refreshed when credentials change. Undefined = not connected yet. */
  supabaseSchema?: Record<string, unknown>;
  /** Explicit allowlist for the customer-facing Supabase read tool. The tool
