...
// delivery. If the email fails (tenant Resend not configured, Resend down, etc.) the
...
const DASHBOARD_URL =
  Deno.env.get("DASHBOARD_URL")?.trim().replace(/\/+$/, "") ||
...
function ticketEmailHtml(t: Ticket, tenant: Tenant): string {
  const ticketUrl = `${DASHBOARD_URL}/?page=tickets&tenant=${encodeURIComponent(tenant.id)}&ticket=${encodeURIComponent(t.id)}`;
  const categoryLabel =
    t.category
      .split("_")
        ...
        <tr>
          <td style="padding:24px;">
            ...
            <div style="margin-top:22px;">
              <a href="${escapeHtml(ticketUrl)}" target="_blank" rel="noopener noreferrer"
...
export async function sendTicketEmail(tenant: Tenant, ticket: Ticket): Promise<TicketEmailResult> {
  ...
  if (!cfg) {
    return { sent: false, provider: "none", error: "Resend is not configured for this tenant — email skipped (ticket still saved)." };
  }
    ...
    const res = await fetch(RESEND_URL, {
      ...
      body: JSON.stringify({
        ...
        text: [
          ...
          ticket.description,
          "",
          `View ticket: ${DASHBOARD_URL}/?page=tickets&tenant=${encodeURIComponent(tenant.id)}&ticket=${encodeURIComponent(ticket.id)}`,
        ].join("\n"),
      }),
    });
    if (!res.ok) {
