...
// delivery. If the email fails (tenant Resend not configured, Resend down, etc.)
// the ticket still exists — the tenant can still see it in their dashboard.
//
...
const DASHBOARD_URL =
  Deno.env.get("DASHBOARD_URL")?.replace(/\/+$/, "") ||
  "https://dashboard-kappa-flax-30.vercel.app";

function resendConfig(tenant: Tenant): { apiKey: string; from: string } | null {
  const apiKey = tenant.resendApiKey?.trim();
  const fromEmail = tenant.resendFromEmail?.trim();

  if (!apiKey || !fromEmail) return null;

  const fromName = tenant.resendFromName?.trim();
...
function ticketEmailHtml(t: Ticket, tenant: Tenant): string {
  // Opens the Tickets section in your dashboard.
  // If/when your dashboard has a dedicated /tickets/:id route,
  // change this to:
  // const ticketUrl = `${DASHBOARD_URL}/tickets/${encodeURIComponent(t.id)}`;
  const ticketUrl = `${DASHBOARD_URL}/tickets`;
  ...
  const rows = [
    ...
    [
      "From",
      `${escapeHtml(t.customerName || "A customer")} &lt;${escapeHtml(
        t.customerEmail,
      )}&gt;`,
    ],
    ...
    .map(
      ([k, v]) =>
        `<tr>` +
        `<td style="padding:6px 12px 6px 0;color:#64748b;vertical-align:top;white-space:nowrap;font-weight:600;">${escapeHtml(
          k,
        )}</td>` +
    ...
    <tr>
      <td align="center">
        ...
        <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e4e4e7;">
          ...
          <tr>
            <td style="background:${escapeHtml(
              tenant.brandColour || "#9c7b4f",
            )};padding:20px 24px;">
          ...
          <tr>
            <td style="padding:24px;">
              ...
              <table role="presentation" cellpadding="0" cellspacing="0" style="margin-bottom:16px;">
                <tr>
                  <td style="font-size:22px;font-weight:800;color:#18181b;">
          ...
          <tr>
            ...
            ">
              Sent automatically by the ${escapeHtml(
                tenant.name,
              )} assistant. Please do not reply to this email.
            </td>
          </tr>
 ...
 * Called AFTER the ticket is persisted, so an email failure cannot lose
 * the actual support ticket.
 */
export async function sendTicketEmail(
  tenant: Tenant,
  ticket: Ticket,
): Promise<TicketEmailResult> {
  const cfg = resendConfig(tenant);
  ...
  if (!cfg) {
    return {
      ...
      provider: "none",
      error:
        "Resend is not configured for this tenant — email skipped (ticket still saved).",
    };
  }

  const to = tenant.supportEmail?.trim();

  if (!to) {
    return {
      sent: false,
      provider: "resend",
      error: `Tenant ${tenant.slug} has no support_email — email skipped.`,
    };
  }
    ...
    const res = await fetch(RESEND_URL, {
      ...
      body: JSON.stringify({
        ...
        text: [
          ...
          ticket.description,
          "",
          `View ticket: ${DASHBOARD_URL}/tickets`,
        ].join("\n"),
      }),
    });
