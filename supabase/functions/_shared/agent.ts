...
export async function runAgent(req: ChatRequest): Promise<ChatResponse> {
  ...
  const legacyCartAdd = allowed.has("add_to_cart") ? parseLegacyCartAdd(req.message) : null;
  const trustedNativeWidgetAction = (req.widgetAction?.type === "cart_add" || legacyCartAdd !== null) && allowed.has("add_to_cart");
  ...
  if (req.widgetAction?.type === "cart_add" || legacyCartAdd) {
    if (!allowed.has("add_to_cart")) return { reply: "Cart is not enabled for this assistant.", products: [], conversationId };
    const p = req.widgetAction?.type === "cart_add" ? (req.widgetAction.payload ?? {}) : (legacyCartAdd ?? {});
    ...
    // Older widget bundles expect this flag and will open their email capture UI,
    // then replay the exact legacy cart command with customerEmail attached.
    if (!knownEmail) {
      return { reply: "To save your cart, please enter your email address.", products: [], conversationId, requiresEmail: true };
    }
 ...
 */
// Backward-compatibility bridge for older/cached widget bundles. Earlier widgets
// emitted Add-to-cart clicks as a natural-language message containing the exact
// server product id. Treat only this tightly-scoped, machine-shaped format as a
// trusted native cart intent. This lets the server complete the action without
// involving the model while newer widgets use widgetAction.cart_add directly.
function parseLegacyCartAdd(message: string): { productId: string; variantId?: string; quantity: number } | null {
  const m = message.trim().match(/^Add\s+.+?\s+\(product\s+([^\s)]+)\)(?:\s+variant\s+([^\s]+))?\s+to\s+my\s+cart\s*$/i);
  if (!m) return null;
  const productId = String(m[1] ?? "").trim();
  if (!productId || productId.length > 200) return null;
  const variantId = String(m[2] ?? "").trim() || undefined;
  return { productId, ...(variantId ? { variantId } : {}), quantity: 1 };
}
