...
function InteractionCard({ interaction, brand, brandTextColor, defaultEmail, disabled, onAction }: {
  ...
  if (interaction.type === "action_form") {
    ...
    return <div style={base}><div style={head}><div style={{fontWeight:800,fontSize:14}}>{interaction.title}</div>{interaction.description?<div style={{fontSize:12,color:COLORS.muted,marginTop:3}}>{interaction.description}</div>:null}</div><div style={{padding:12,display:"grid",gap:9}}>{fields.map(([key,rule]:any)=>{const label=rule.title||rule.description||key.replace(/_/g," "); if(Array.isArray(rule.enum)) return <label key={key} style={{display:"grid",gap:5,fontSize:11,fontWeight:700,color:COLORS.muted}}>{label}<select style={inputStyle} value={values[key]??""} onChange={(e)=>setValues(v=>({...v,[key]:e.target.value}))}><option value="">Choose…</option>{rule.enum.map((x:any)=><option key={String(x)} value={String(x)}>{String(x)}</option>)}</select></label>; if(rule.type==="array") return <label key={key} style={{display:"grid",gap:5,fontSize:11,fontWeight:700,color:COLORS.muted}}>{label}<input style={inputStyle} type={rule.items?.format==="email"?"email":"text"} value={Array.isArray(values[key])?values[key].join(", "):values[key]??""} onChange={(e)=>setValues(v=>({...v,[key]:e.target.value.split(",").map((x:string)=>x.trim()).filter(Boolean)}))}/></label>; if(key==="text"||key==="html"||rule.type==="textarea") return <label key={key} style={{display:"grid",gap:5,fontSize:11,fontWeight:700,color:COLORS.muted}}>{label}<textarea style={{...inputStyle,minHeight:88,resize:"vertical"}} value={values[key]??""} onChange={(e)=>setValues(v=>({...v,[key]:e.target.value}))}/></label>; return <label key={key} style={{display:"grid",gap:5,fontSize:11,fontWeight:700,color:COLORS.muted}}>{label}<input style={inputStyle} type={rule.format==="email"?"email":rule.type==="number"||rule.type==="integer"?"number":"text"} value={values[key]??""} onChange={(e)=>setValues(v=>({...v,[key]:rule.type==="number"||rule.type==="integer"?Number(e.target.value):e.target.value}))}/></label>})}<button disabled={disabled||missing} style={{...primary,opacity:missing ? .45 : 1}} onClick={()=>onAction(interaction.requireConfirmation?`Review ${interaction.actionName}`:`Continue with ${interaction.actionName}`,{type:"connector_action_submit",payload:{actionId:interaction.actionId,input:values}})}>{interaction.submitLabel||"Continue"}</button></div></div>;
  }
