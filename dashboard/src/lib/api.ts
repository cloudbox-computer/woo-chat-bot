...
export function runPlatformTest(tenantId:string,id:string){return request<{ok:boolean;passed:boolean;response:string;score:number;checks:Array<{expected:string;matched:boolean;score:number}>;forbiddenHits:string[]}>(platformQuery(tenantId,"tests",id),{method:"POST",body:JSON.stringify({id,operation:"run"})});}
