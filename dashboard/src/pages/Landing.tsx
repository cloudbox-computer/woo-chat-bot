...
const plans = [
  ...
  { key: "growth" as const, name: "Growth", price: "£79", desc: "For teams that want AI connected to real business systems.", popular: true, items: ["3 AI assistants", "2,500 conversations / month", "Everything in Starter", "Live integrations & built-in AI actions", "Customer-safe actions", "Per-assistant restricted action permissions", "Team access & human takeover", "Full dashboard analytics"] },
  { key: "scale" as const, name: "Scale", price: "£199", desc: "For higher-volume businesses, agencies and advanced automation.", items: ["10 AI assistants", "10,000 conversations / month", "Everything in Growth", "Custom API actions", "Advanced team permissions", "Audit log & operations", "Enterprise controls", "Higher usage limits"] },
];
...
const faqs = [
  ...
  ["Can every assistant use every connected action?", "No. Connecting a service does not automatically grant every assistant permission to use it. Actions can be scoped per assistant, and restricted write actions require an explicit administrator grant. Growth and Scale include per-assistant restricted-action controls; Scale also adds custom API actions and advanced permissions."],
...
export default function Landing({ onLogin, onChoosePlan }: { onLogin: () => void; onChoosePlan: (plan: "starter" | "growth" | "scale") => void }) {
  React.useEffect(() => {
    document.title = "ZoChat — AI Customer Support, Integrations & Business Actions";
  }, []);
  return <div className="landing">
    ...
    <main id="top">
      <section className="lp-hero">
        ...
        <p className="lp-lead">Give customers instant, useful answers from your website, knowledge and connected systems. ZoChat can answer questions, find products, check orders, book appointments, create leads and perform approved business actions — while staying inside the rules you set.</p>
        <div className="lp-actions"><button className="lp-primary" onClick={onLogin}>Start your 14-day free trial <span>→</span></button><a className="lp-secondary" href="#how">See how it works</a></div>
      ...
      <section className="lp-proof"><span>BUILT FOR THE TOOLS YOUR BUSINESS ALREADY USES</span><div><b>WooCommerce</b><b>Shopify</b><b>Calendly</b><b>HubSpot</b><b>Slack</b><b>Resend</b></div></section>
      ...
      <section className="lp-dark" id="how"><div className="lp-section-kicker">HOW IT WORKS</div><h2>From signup to helpful<br/>customer conversations.</h2><div className="lp-steps"><article><b>01</b><h3>Teach it your business</h3><p>Add your website, business context, FAQs and knowledge. Define exactly what the assistant should and should not discuss.</p></article><article><b>02</b><h3>Connect live capabilities</h3><p>Connect the systems you need. ZoChat can use approved read and write actions while credentials, provider details and permission checks stay server-side.</p></article><article><b>03</b><h3>Embed and improve</h3><p>Add one script to your site, review conversations and tickets in the dashboard, then refine knowledge and behaviour as you learn.</p></article></div></section>

      <section className="lp-section lp-integrations" id="integrations"><div className="lp-copy"><div className="lp-section-kicker">INTEGRATIONS + CONTROLLED ACTIONS</div><h2>Connect your business.<br/><em>Control exactly what AI can do.</em></h2><p>ZoChat works through server-approved capabilities rather than handing the model unrestricted access to your systems. Connect commerce, CRM, scheduling, support, messaging and automation tools, then decide which assistants can use each action.</p><div className="lp-checks"><span>✓ Product, order & customer lookups</span><span>✓ Appointment booking & lead creation</span><span>✓ Support tickets & CRM actions</span><span>✓ Messaging and email integrations</span><span>✓ Per-assistant action permissions</span><span>✓ Restricted writes disabled by default</span></div><div className="lp-action-security"><strong>You stay in control of every action.</strong><p>Connecting an integration does not automatically authorise every assistant. Sensitive actions can be assigned individually, restricted writes require an explicit grant, and confirmation rules are enforced server-side.</p></div></div><div className="lp-orbit"><div className="lp-core">◆<b>ZoChat</b><small>AI action layer</small></div>{["WooCommerce","Calendly","HubSpot","Resend","Slack","Custom API"].map((x,i)=><div key={x} className={`lp-node n${i}`}>{x}</div>)}</div></section>

      <section className="lp-section lp-pricing" id="pricing"><div className="lp-section-kicker">SIMPLE PRICING</div><h2>Start small. <em>Scale when support grows.</em></h2><p className="lp-section-intro">Straightforward monthly plans with a 14-day free trial. Card details are handled securely by Stripe and billing starts after the trial unless you cancel.</p><div className="lp-price-grid">{plans.map(p=><article className={p.popular?"popular":""} key={p.name}>{p.popular&&<div className="lp-popular">MOST POPULAR</div>}<h3>{p.name}</h3><p>{p.desc}</p><div className="lp-price"><b>{p.price}</b><span>/ month<br/><small>ex VAT</small></span></div><button onClick={()=>onChoosePlan(p.key)}>Start free trial <span>→</span></button><ul>{p.items.map(x=><li key={x}>✓ {x}</li>)}</ul></article>)}</div><p className="lp-price-note">Need custom volume, dedicated onboarding or enterprise controls? Choose a plan to create your account and discuss a tailored setup with the team.</p></section>
