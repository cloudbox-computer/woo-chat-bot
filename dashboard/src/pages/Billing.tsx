...
export default function BillingPage({ tenantId }: { tenantId: string }) {
  ...
  return <div className="page billing-page">
    <div className="billing-head"><div><h1>Billing & plan</h1><p className="desc">Manage your ZoChat subscription, 14-day trial, monthly allowance and Stripe billing.</p></div>{billing?.hasCustomer&&canManage&&<button className="btn ghost" onClick={portal} disabled={!!busy}>{busy==="portal"?"Opening…":"Manage billing in Stripe"}</button>}</div>
    ...
    <div className="billing-plan-grid">{plans.map(plan=>{
      ...
      return <article className={`billing-plan card ${(requestedPlan?plan.key===requestedPlan:plan.key==="growth")?"featured":""}`} key={plan.key}>
        {plan.key==="growth"&&<span className="billing-badge">MOST POPULAR</span>}<h2>{plan.name}</h2><div className="billing-price"><b>£{plan.monthlyPriceGbp}</b><span>/ month<br/><small>ex VAT</small></span></div><ul><li>✓ {plan.maxAssistants} AI assistant{plan.maxAssistants!==1?"s":""}</li><li>✓ {plan.conversationLimit.toLocaleString()} conversations / month</li><li>✓ Website, knowledge & support tickets</li>{plan.key!=="starter"&&<><li>✓ Live integrations & built-in AI actions</li><li>✓ Customer-safe actions & per-assistant restricted action grants</li><li>✓ Business-data tools, team access & human takeover</li><li>✓ Full dashboard analytics</li></>}{plan.key==="scale"&&<><li>✓ Custom API actions & advanced permissions</li><li>✓ Audit log, operations & enterprise controls</li></>}</ul>{canManage ? current ? <button className="btn ghost" disabled>Current plan</button> : active ? <button className="btn primary" onClick={portal} disabled={!!busy}>Change plan in Stripe</button> : <button className="btn primary" onClick={()=>checkout(plan.key)} disabled={!!busy}>{busy===plan.key?"Opening checkout…":`Start ${plan.name} trial`}</button> : <button className="btn ghost" disabled>Owner/admin required</button>}</article>
    })}</div>
